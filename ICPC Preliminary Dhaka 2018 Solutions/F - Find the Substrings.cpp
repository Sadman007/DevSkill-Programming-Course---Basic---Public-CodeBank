...
int duplicateSubStrings[MAX+5];

void pre() {
    po[0] = 1;
    for(int i=1;i<MAX;i++) {
        po[i] = (po[i-1]*26);
        if(po[i]<0) po[i]+=MOD+MOD;
        if(po[i]>MOD) po[i]%=MOD;
    }
    for(int i=1;i<MAX;i++) po[i]+=po[i-1];
}

ll get(ll X) {
    ll r = (1LL*n*X - (1LL*X*(X+1)/2) + X)%MOD;
    return r;
}

ll getFull(ll L,ll R) {
    ll r = (get(R)-get(L-1)+MOD+MOD)%MOD;
    return r;
}
...
int main() {
    ...
    while(test--) {
        ...
        LcpArray();
        for(int i=0;i<n;i++) {
            duplicateSubStrings[lcp[i]+1]--;
            duplicateSubStrings[1]++;
        }
        for(int i=0;i<=n;i++) duplicateSubStrings[i]+=duplicateSubStrings[i-1];
        for(int i=0;i<=n;i++) duplicateSubStrings[i]+=duplicateSubStrings[i-1];
        scanf("%d",&query);
        while(query--) {
            int L,R;
            scanf("%d%d",&L,&R);
            ll all = (po[R]-po[L-1]+MOD+MOD)%MOD;
            ll full = (getFull(L,R)-(duplicateSubStrings[R]-duplicateSubStrings[L-1])+MOD+MOD)%MOD;
            ll res = (all-full+MOD+MOD)%MOD;
            while(res<0) res+=MOD+MOD;
            if(res>MOD) res%=MOD;
            printf("%lld\n",res);
        }
        CLR();
